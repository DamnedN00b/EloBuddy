...
namespace Lazy_Illaoi
{
    internal class Spells
    {
        ...
        public static void CastQ()
        {
            ...
            var ghost =
                ObjectManager.Get<Obj_AI_Minion>()
                    .FirstOrDefault(x => x.IsValidTarget(Q.Range) && x.HasBuff("illaoiespirit"));
            if (target != null)
            {
                ...
                if ((Init.ComboMenu["useEQ"].Cast<CheckBox>().CurrentValue && ePred.HitChance >= HitChance.Medium &&
                     E.IsReady()) || !Q.IsReady()) return;
            }

            if (target != null && ghost != null)
            {
                var predPos = Prediction.Position.PredictLinearMissile(target, Q.Range, Q.Width, Q.CastDelay, Q.Speed,
                    int.MaxValue, null, true);
            ...
            if (target == null || ghost != null) return;
            {
                ...
                if ((target.Health < Player.GetSpellDamage(target, SpellSlot.Q) && predPos.HitChance >= HitChance.Medium) ||
                    // Cast on target
        ...
        public static void CastE()
        {
            ...
            if (!E.IsReady() ||
                !target.IsValidTarget() || target.HasBuffOfType(BuffType.Invulnerability) || target.IsDead ||
                target.IsZombie)
                return;
        ...
        public static void CastR()
        {
            var ghost =
                ObjectManager.Get<Obj_AI_Minion>()
                    .FirstOrDefault(x => x.HasBuff("illaoiespirit") && x.Distance(Player) < R.Range);
            var targets =
                EntityManager.Heroes.Enemies.Where(
                    x =>
                        x.IsValidTarget(R.Range) && !x.HasBuffOfType(BuffType.Invulnerability) && !x.IsDead &&
                        !x.IsZombie);

            if (!R.IsReady()) return;

            var aiHeroClients = targets as AIHeroClient[] ?? targets.ToArray();
            if ((Init.ComboMenu["useRghost"].Cast<CheckBox>().CurrentValue && ghost == null) ||
                aiHeroClients.Count() < Init.ComboMenu["useR#"].Cast<Slider>().CurrentValue) return;
            {
                R.Cast();
            }
        }
